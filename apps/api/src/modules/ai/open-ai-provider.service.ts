...
export class OpenAiProviderService implements AiProvider {
  ...
  private buildSystemInstructions(input: GenerateDailyPlanInput, retry: boolean) {
    return [
      ...
      'requestedExerciseCount, minExerciseCount, maxExerciseCount, workoutDurationMinutes, and workoutVolumePlan are deterministic backend constraints.',
      ...
      'Exercise prescription fields are machine-readable and must stay in ASCII English for every output locale; localize only user-facing names, cues, safety notes, and notes.',
      'For strength: sets must be exactly "1"-"5", reps exactly like "10", "8-10", or "8-10 per side", rest exactly like "60 seconds" or "45-60 seconds", and duration must be an empty string.',
      'For mobility, cardio, and recovery: sets, reps, and rest must be empty strings; duration must use exact English units like "30 seconds", "2 minutes", or a valid range and fit within workoutDurationMinutes.',
