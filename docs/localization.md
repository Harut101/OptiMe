...
Use `getHealthProviderLabel` for stable provider enums: `APPLE_HEALTH`, `HEALTH_CONNECT`, `WHOOP`, `GARMIN`, `MANUAL`, and `MOCK`. Do not concatenate provider names with English status fragments.

Weight tracking strings live under `weight` in all supported mobile locales. Required concepts include current weight, target weight, starting weight, update weight, history, manual entry, invalid weight, future-plans-only copy, and neutral progress wording. Do not use shame, urgency, or “behind target” wording.
