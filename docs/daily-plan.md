...
`DailyPlanJson.contextNotes` is optional and backward compatible. It can contain wearable, training-load, and recovery notes generated by the backend after provider output. Mobile uses these notes for compact Today guidance and Plan Details explanations.
