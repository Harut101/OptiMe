...
Onboarding now presents primary goal and app mode as selectable cards with short supportive descriptions. This is a UI-only change; the saved `PrimaryGoal`, `GoalType`, `GoalImpactMode`, and `appMode` values are unchanged.
